...
export function periodTimeTemplate(index, { temp, date, icon, description}) {
  return `
    <li class="dayWeather-item" id="dayWeather-item-${index}">
...
export function createPeriodTime(weather, indexWeather) {
  ...
  return createDOM(periodTimeTemplate(indexWeather, config))
}